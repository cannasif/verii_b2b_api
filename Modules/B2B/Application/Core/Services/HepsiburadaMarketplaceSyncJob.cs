...
public sealed class HepsiburadaMarketplaceSyncJob : IHepsiburadaMarketplaceSyncJob
{
    private const string RecurringJobId = "marketplace-hepsiburada-sync-job";
    private readonly IRepository<MarketplaceSyncEvent> _events;
    private readonly IHepsiburadaMarketplaceClient _client;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IJobFailureLogWriter _jobFailureLogWriter;
    private readonly ILogger<HepsiburadaMarketplaceSyncJob> _logger;
    private readonly HepsiburadaMarketplaceOptions _options;

    public HepsiburadaMarketplaceSyncJob(
        IRepository<MarketplaceSyncEvent> events,
        IHepsiburadaMarketplaceClient client,
        IUnitOfWork unitOfWork,
        IJobFailureLogWriter jobFailureLogWriter,
        ILogger<HepsiburadaMarketplaceSyncJob> logger,
    ...
    public async Task<int> RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            await _jobFailureLogWriter.WriteAsync(
                ...
                $"{typeof(HepsiburadaMarketplaceSyncJob).FullName}.RunAsync",
                "HepsiburadaMarketplaceSyncJobUnhandled",
                ex,
    ...
    private async Task<int> SendPendingAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var item in pendingEvents)
        {
            ...
            item.ExternalBatchId = result.TransactionId ?? item.ExternalBatchId;
            ...
            _logger.LogInformation(
                "Hepsiburada marketplace event sent EventId={EventId} Operation={OperationType} Status={Status} TransactionId={TransactionId}",
                item.Id,
                item.OperationType,
                item.Status,
    ...
    private async Task<int> PollQueuedAsync(CancellationToken cancellationToken)
    {
        ...
        {
            var result = await _client.GetTransactionStatusAsync(item, cancellationToken);
            item.ResponseJson = result.ResponseJson;
            if (!result.IsFinal)
            {
                item.SetUpdatedInfo();
            ...
            _logger.LogInformation(
                "Hepsiburada marketplace transaction polled EventId={EventId} Operation={OperationType} Status={Status} TransactionId={TransactionId}",
                item.Id,
                item.OperationType,
                item.Status,
                item.ExternalBatchId);
    ...
    private IQueryable<MarketplaceSyncEvent> BuildBaseQuery(bool tracking) =>
        _events.Query(tracking)
            ...
            .Where(x => !x.IsDeleted && x.Channel != null && x.Channel.ProviderKey == "Hepsiburada");
